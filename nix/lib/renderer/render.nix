...
in {
  flake.lib.checkAssertions = device: let
    failed = builtins.filter (a: !a.assertion) device.assertions;
  in
    if failed == []
    then null
    else
      throw ''
        nixco: ${toString (builtins.length failed)} assertion(s) failed for device '${device.hostname}':
  ...
  flake.lib.render = device:
    builtins.seq (self.lib.checkAssertions device) (
